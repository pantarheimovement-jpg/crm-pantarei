...
export default function WaTemplateComposer({ templates, selected, onSelectTemplate, courses = [], course, date, link, onChangeCourse, onChangeDate, onChangeLink }) {
  ...
  // בחירת קורס מהדרופדאון דורסת את שלושת השדות; אחר כך כל שדה ניתן לעריכה חופשית.
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">בחרי קורס (למילוי אוטומטי)</label>
        <select value={selectedCourseName} onChange={(e) => handleCourseSelect(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">שם הקורס בהודעה</label>
          <input type="text" value={course} onChange={(e) => onChangeCourse(e.target.value)} placeholder="למשל: נענע" className="w-full px-4 py-2 border border-gray-300 rounded-lg" />
        </div>
        <div>
      ...
      <p className="text-xs text-gray-600">השם ({'{{1}}'}) מתמלא אוטומטית לכל נמען משם המנוי. בחירת קורס ממלאת את שלושת השדות — וכל אחד ניתן לעריכה חופשית לפני שליחה (למשל לקצר את שם הקורס ל"נענע").</p>
    </div>
  );
}