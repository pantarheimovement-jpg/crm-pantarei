          ...
          <div className="p-4 md:p-6">
            ...
            {activeTab === 'automation' && automationSettings && (
              <div className="space-y-6">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">📱 תגובה אוטומטית לוואטסאפ נכנס (לידים חדשים)</h3>
                  <div className="bg-green-50 border border-green-200 rounded-lg p-5 space-y-4">
                    <p className="text-sm text-gray-700">
                      כשמישהו חדש שולח הודעה בוואטסאפ, המערכת תיצור אוטומטית ליד חדש ותשלח הודעת תגובה.
                    </p>
                    <div className="flex items-center gap-3">
                      <input
                        type="checkbox"
                        checked={automationSettings.whatsapp_auto_reply_enabled !== false}
                        onChange={(e) => setAutomationSettings({...automationSettings, whatsapp_auto_reply_enabled: e.target.checked})}
                        className="w-5 h-5 text-green-600"
                      />
                      <label className="text-sm font-medium text-gray-700">הפעל תגובה אוטומטית לוואטסאפ נכנס</label>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                      ...
                      <textarea
                        value={automationSettings.whatsapp_auto_reply || 'הי {{name}}, קיבלנו את פנייתך 💜 אנחנו נדאג ליצור איתך קשר בהקדם. סטודיו פנטהריי'}
                        onChange={(e) => setAutomationSettings({...automationSettings, whatsapp_auto_reply: e.target.value})}
                        rows="3"
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                      />
                      <p className="text-xs text-gray-500 mt-1">💡 השתמשי ב-{'{{name}}'} לשם השולח מוואטסאפ</p>
                    </div>
                  </div>
                </div>

                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">🌐 הודעת וואטסאפ לליד חדש מטופס באתר</h3>
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-5 space-y-4">
                    <p className="text-sm text-gray-700">
                      כשליד חדש מגיע מטופס באתר (Elementor), המערכת שולחת הודעת וואטסאפ ייעודית <strong>עם שם הקורס</strong>.
                    </p>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        הודעה לליד מהאתר
                      ...
                      <textarea
                        value={automationSettings.whatsapp_new_lead_from_site || 'הי {{name}}, קיבלנו את פנייתך בנוגע ל{{course}} 💜 ניצור איתך קשר בהקדם. סטודיו פנטהריי'}
                        onChange={(e) => setAutomationSettings({...automationSettings, whatsapp_new_lead_from_site: e.target.value})}
                        rows="3"
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg"
                      />
                      <p className="text-xs text-gray-500 mt-1">💡 השתמשי ב-{'{{name}}'} לשם, וב-{'{{course}}'} לשם הקורס. אם אין קורס, תישלח ההודעה הרגילה למעלה.</p>
                    </div>
                  </div>
                </div>
